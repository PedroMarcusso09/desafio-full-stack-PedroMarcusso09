...
export const ContactsProvider = ({ children }: { children: ReactNode }) => {
   ...
   const deleteContacts = async (currentContactId: string) => {
      try {
         ...
         setContactsList((contactsList) => contactsList.filter(currentContact => currentContact.id !== currentContactId));
      } catch (error) {
         console.log(error)
      }
   }
   ...
   const editContacts = async (contactId: string, updatedContactData: UpdateContactFormData) => {
      try {
         const token = localStorage.getItem("@TOKEN");

         const response = await api.patch(`/contacts/${contactId}`, updatedContactData, {
            headers: {
               Authorization: `Bearer ${token}`
            },
         });
   ...
   return (
      <ContactsContext.Provider value={{ loading, contactsList, createContacts, deleteContacts, editContacts }}>
         {children}
